class Content:
  ...
  def print(self):
    print('New article found for topics: {}'.format(self.topic))
...
class Crawler:
  ...
  def search(self, topic, site):
    """
    Mencatat semua halaman yang telah ditemukan yang dicari pada situs tertentu dan
    pada topik tertentu
    """
    ...
    for result in searchResults:
      ...
      # Cek untuk mengetahui apakah ini absolut atau relatif url
      if(site.absoluteUrl):
        bs = self.getPage(url)
      else:
        bs = self.getPage(site.url + url)
      if bs is None:
        print("Terjadi masalah pada url. Skip!")
        return
